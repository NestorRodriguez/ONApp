...
@Component({
  ...
  styleUrls: ['./pozo.page.scss'],
})
export class PozoPage implements OnInit {
  ...
  async presentLoading() {
    ...
    try {
      this.listaVerificacion = await this.storageToJson.getJsonStorageList('menuascensores', 'pozo');
  ...
  async SaveModel(ruta: string) {
    ...
    if (ruta === 'icon') {
      const mensaje = 'Lista de verificación pozo guardada con éxito!';
